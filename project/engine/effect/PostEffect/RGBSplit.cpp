...
namespace Tako {
  ...
  void RGBSplit::CreateRootSignature()
  {
    ...
    samplerDesc[0].AddressU = D3D12_TEXTURE_ADDRESS_MODE_CLAMP; // 画面端でクランプ（ラップアラウンド防止）
    samplerDesc[0].AddressV = D3D12_TEXTURE_ADDRESS_MODE_CLAMP; // 画面端でクランプ（ラップアラウンド防止）
