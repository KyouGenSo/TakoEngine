...
namespace Tako {
  ...
  void Dissolve::CreateRootSignature()
  {
    ...
    // mask texture 用の DescriptorRange の設定。
    ...
    descriptorRangeForMask[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // SRV を使う
    descriptorRangeForMask[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND; // Offset を自動計算

    // base texture 用の DescriptorRange の設定。
    ...
    descriptorRangeForBaseTex[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // SRV を使う
    descriptorRangeForBaseTex[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND; // Offset を自動計算
