...
void LuminanceBasedOutline::Initialize(DX12Basic* dx12, std::string shaderName)
{
  IPostEffect::Initialize(dx12, shaderName);
  CreateCBV();
}

void LuminanceBasedOutline::Apply(uint32_t inputSrvIndex, D3D12_CPU_DESCRIPTOR_HANDLE outputRtvHandle, uint32_t depthSrvIndex, Vector4 clearColor)
{
