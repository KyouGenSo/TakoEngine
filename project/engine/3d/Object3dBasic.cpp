...
void Object3dBasic::SetSkinningRenderSetting()
{
  // ルートシグネチャの設定
  m_dx12_->GetCommandList()->SetGraphicsRootSignature(skinningRootSignature_.Get());

  // パイプラインステートの設定
  m_dx12_->GetCommandList()->SetPipelineState(skinningPipelineState_.Get());
...
void Object3dBasic::CreateSkinningRootSignature()
{
  HRESULT hr;
  ...
  descriptorRangeForWell[0].BaseShaderRegister = 0; // レジスタ番号
  descriptorRangeForWell[0].NumDescriptors = 1; // ディスクリプタ数
  descriptorRangeForWell[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; // SRVを使う
  descriptorRangeForWell[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND; // Offsetを自動計算
  ...
  rootParameters[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // バーテックスシェーダーで使う
  rootParameters[8].DescriptorTable.pDescriptorRanges = descriptorRangeForWell; // ディスクリプタレンジを設定
  rootParameters[8].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForWell); // レンジの数
  ...
  hr = m_dx12_->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(), signatureBlob->GetBufferSize(), IID_PPV_ARGS(skinningRootSignature_.GetAddressOf()));
  signatureBlob->GetBufferSize(), IID_PPV_ARGS(skinningRootSignature_.GetAddressOf());
  assert(SUCCEEDED(hr));
}

void Object3dBasic::CreateSkinningPSO()
{
  HRESULT hr;
  ...
  // shaderのコンパイル
  Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob = m_dx12_->CompileShader(L"resources/shaders/SkinningObject3d.VS.hlsl", L"vs_6_0");
  assert(vertexShaderBlob != nullptr);
  ...
  // 実際に生成
  hr = m_dx12_->GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&skinningPipelineState_));
  assert(SUCCEEDED(hr));
}
